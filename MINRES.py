...
#    normHy2 = tau**2
    while flag == flag0:
        ...
        if torch.norm(rt) / norm_b < rtol:
            ...
            return xl, iters, rtl, dType, torch.norm(b - Ax(A, xl) - rtl), torch.abs(torch.dot(b, vn)) / (norm_b * torch.norm(vn))
        ...
        if gamma2 > isZero:
            ...
            maxit += 1
            return x, iters, rt, dType, torch.norm(b - Ax(A, x) - rt), torch.abs(torch.dot(b, vn)) / (norm_b * torch.norm(vn))
